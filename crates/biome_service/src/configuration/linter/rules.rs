...
pub struct Nursery {
    ...
    #[bpaf(long("enforce-foo-bar"), argument("on|off|warn"), optional, hide)]
    #[serde(skip_serializing_if = "Option::is_none")]
    pub enforce_foo_bar: Option<RuleConfiguration>,
...
impl MergeWith<Nursery> for Nursery {
    fn merge_with(&mut self, other: Nursery) {
        if let Some(enforce_foo_bar) = other.enforce_foo_bar {
            self.enforce_foo_bar = Some(enforce_foo_bar);
        }
...
impl Nursery {
    ...
    pub(crate) fn get_enabled_rules(&self) -> IndexSet<RuleFilter> {
        let mut index_set = IndexSet::new();
        if let Some(rule) = self.enforce_foo_bar.as_ref() {
            if rule.is_enabled() {
                index_set.insert(RuleFilter::Rule(Self::GROUP_NAME, Self::GROUP_RULES[0]));
            }
        }
    ...
    pub(crate) fn get_rule_configuration(&self, rule_name: &str) -> Option<&RuleConfiguration> {
        match rule_name {
            "enforceFooBar" => self.enforce_foo_bar.as_ref(),
